...
namespace CollectionViewDemos.ViewModels
{
    public class GroupedAnimalsViewModel
    {
        ...
        private int _newBearIterator = 0;
        public ICommand AddNewBearOnMainThread => new Command(() =>
        {
            
            Device.BeginInvokeOnMainThread(() =>
            {
                var bears = Animals.Single(x => x.Name == "Bears");
                bears.Add(new Animal()
                {
                    Name = $"New Bear {_newBearIterator++}",
                    Location = $"New Location {_newBearIterator}",
                    Details = $"New Details {_newBearIterator}",
        ...
        public ICommand AddNewBear => new Command(() =>
        {
